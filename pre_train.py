...
def pre_train(args):
    ...
    from models.mae_1D_restructed_by_all import MAE_restructed_by_all
    from models.mae_1D_without_mask import MAE_without_mask
    from models.mae_1D_linearmask import MAE_linearmask
    import models.models_mae_1D as mae_fb_1D

    model = mae_fb_1D.mae_vit_signal_patch48_mask25()
...
def pre_train_procedure(args,model : nn.Module ,model_save_dir : str) -> None:
    ...
    train_dataloader = DataLoader(train_dataset,batch_size=config.batch_size,shuffle=False,num_workers=6 if config.onserver else 1)
    
    len_train_dataset = len(train_dataset)
    val_dataset = ECGDatasetForPreTrain(data_path=config.pretrain_val_list)
    val_dataloader = DataLoader(val_dataset,batch_size=config.batch_size,shuffle=False,num_workers=4 if config.onserver else 1)
