...
class Attention(nn.Module):
    def __init__(self, dim,ifplot=False,num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0.,id = -1):
        super().__init__()
        self.id = id
        self.ifplot = ifplot
    ...
    def forward(self, x):
        ...
        if self.id > 0:
            np.save('/home/ECG_AI/N_MAE_ECG/MAE_ECG/plot/attns/attn{}.npy'.format(self.id),attn.numpy())
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm,id = -1):
        super().__init__()
        self.id = id
        self.norm1 = norm_layer(dim)
        self.attn = Attention(dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop,id = self.id)
...
class MaskedAutoencoderViT(nn.Module):
    ...
    def __init__(self, img_size=2500, patch_size=50, in_chans=1,
                 ...
                 mlp_ratio=4., norm_layer=nn.LayerNorm, norm_pix_loss=False,mask_ratio = 0.75,mask = 'random'):
        super().__init__()
        ...
        self.decoder_blocks = nn.ModuleList([
            Block(decoder_embed_dim, decoder_num_heads, mlp_ratio, qkv_bias=True, norm_layer=norm_layer,id=i)
...
def mae_vit_signal_patch12_enc40_dec20d8b_m75(**kwargs):
    model = MaskedAutoencoderViT(
        ...
        decoder_embed_dim=20,decoder_depth=8,decoder_num_heads=10,
        mlp_ratio=2, norm_layer=partial(nn.LayerNorm, eps=1e-6),mask_ratio=0.75, **kwargs
    )
    return model
def mae_vit_signal_patch12_enc40_dec20d8b_m50(**kwargs):
    model = MaskedAutoencoderViT(
        img_size = 2400,patch_size=12,embed_dim=40,depth=12,num_heads=10,
        decoder_embed_dim=20,decoder_depth=8,decoder_num_heads=10,
        mlp_ratio=2, norm_layer=partial(nn.LayerNorm, eps=1e-6),mask_ratio = 0.50,**kwargs
    )
    return model
def mae_vit_signal_patch12_enc40_dec20d8b_m25(**kwargs):
    model = MaskedAutoencoderViT(
        img_size = 2400,patch_size=12,embed_dim=40,depth=12,num_heads=10,
        decoder_embed_dim=20,decoder_depth=8,decoder_num_heads=10,
        mlp_ratio=2, norm_layer=partial(nn.LayerNorm, eps=1e-6),mask_ratio = 0.25,**kwargs
    )
    return model
def mae_vit_signal_patch12_enc40_dec20d8b_m75_mean(**kwargs):
    model = MaskedAutoencoderViT(
        img_size = 2400,patch_size=12,embed_dim=40,depth=12,num_heads=10,
        decoder_embed_dim=20,decoder_depth=8,decoder_num_heads=10,
        mlp_ratio=2, norm_layer=partial(nn.LayerNorm, eps=1e-6),mask_ratio=0.75,mask='mean', **kwargs
    )
    return model
