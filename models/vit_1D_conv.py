...
class ViT1D_conv(nn.Module):
    def __init__(self, *, signal_lenth, patch_lenth, num_classes, dim, depth, heads, mlp_dim, pool = 'cls', channels = 1, dim_head = 64, dropout = 0., emb_dropout = 0.):
        super().__init__()
        ...
        self.to_patch_embedding = nn.Sequential(
            ...
            nn.Linear(patch_dim, patch_lenth)
            # nn.Conv1d(in_channels=patch_dim,out_channels=dim,kernel_size=1)
        )
        #o_o#
        self.pos_embedding = nn.Parameter(torch.randn(1, num_patches + 1, patch_lenth))
